execute as @a[scores={unbreaking=1}] store result score @s variables run clear @s minecraft:player_head 0
...
execute as @a[scores={unbreaking=1}] at @s if entity @e[limit=1,distance=..5,type=minecraft:item] if score @s variables matches 10.. run clear @s minecraft:player_head 10
