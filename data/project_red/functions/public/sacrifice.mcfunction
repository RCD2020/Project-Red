execute as @a[scores={sacrifice=1}] store result score count variables run clear @s minecraft:golden_apple{CustomModelData:100} 0

# Shows list of options
execute as @a[scores={sacrifice=1}] run tellraw @s {"text":"--- Sacrifice ---","color":"red","bold":true,"underlined":true}
execute as @a[scores={sacrifice=1}] run tellraw @s [{"text":"You must get ","italic":true},{"score":{"name":"sacrifice_req","objective":"constants"},"italic":true},{"text":" souls to perform a ritual","italic":true}]
